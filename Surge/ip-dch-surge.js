...
(async () => {
  ...
  const content = [
    `本地：${local.ip} · ${local.loc} · ${local.isp}`,
    `落地：${landing.ip} · ${landing.loc} · ${landing.native}`,
    `风险：IPPure ${landing.ippure} / ipapi ${landing.ipapi}`,
    `解锁：GPT ${okMark(gpt)} ${gpt}｜Gemini ${okMark(gemini)} ${gemini}｜YouTube ${okMark(yt)} ${yt}`,
    `      Netflix ${okMark(nf)} ${nf}｜TikTok ${okMark(tt)} ${tt}`,
    `更新：${hh}:${mm} · ${cost}s`
  ].join("\n");
