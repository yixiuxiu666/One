...
(async () => {
  ...
  const title = `DCH ${POLICY || "Proxy"} · ${sevText(maxSev)}`;
  const clean = s => String(s || "").replace(/[\u{1F1E6}-\u{1F1FF}\u{1F300}-\u{1FAFF}]/gu, "").replace(/\s+/g, " ").trim();
  const shortLoc = s => clean(s).replace(/中国 /g, "CN ").replace(/日本|Japan/g, "JP").replace(/美国|United States/g, "US").replace(/新加坡|Singapore/g, "SG").replace(/香港|Hong Kong/g, "HK");
  const shortNative = clean(landing.native).replace("原生住宅", "住宅").replace("商业机房", "机房");
  const ok = v => (v === "不可用" || v === "CN") ? "×" : "✓";
  const content = [
    `本地 ${local.ip} ｜ ${shortLoc(local.loc)} ${clean(local.isp)}`,
    `落地 ${landing.ip} ｜ ${shortLoc(landing.loc)} ${shortNative}`,
    `风险 IPP ${landing.ippure} ｜ API ${landing.ipapi}`,
    `解锁 GPT ${ok(gpt)}${gpt}  Gem ${ok(gemini)}${gemini}  YT ${ok(yt)}${yt}`,
    `     NF ${ok(nf)}${nf}  TT ${ok(tt)}${tt} ｜ ${hh}:${mm} ${cost}s`
  ].join("\n");
